...
class MainActivity : AppCompatActivity() {
    ...
    private fun guardarRutaEnDB(nombreRuta: String) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                // Save the route and get its ID
                val rutaId = db.addRuta(nombreRuta)

                // Prepare the list of points (the optimal route)
                val puntosDeRuta = rutaOptima.map { puntos[it] }

                // Create a list of model.Punto objects for the database
                val puntosParaGuardar = puntosDeRuta.mapIndexed { index, p1 ->
                    var controlX: Float? = null
                    var controlY: Float? = null

                    // Calculate control point for the segment to the next point
                    if (index < puntosDeRuta.size - 1) {
                        val p2 = puntosDeRuta[index + 1]
                        controlX = (p1.x + p2.x) / 2f + (p2.y - p1.y) * 0.2f
                        controlY = (p1.y + p2.y) / 2f - (p2.x - p1.x) * 0.2f
                    }

                    // Create the Punto model object
                    com.example.p1.model.Punto(
                        id = 0, // ID is autogenerated by the database
                        rutaId = rutaId,
                        x = p1.x,
                        y = p1.y,
                        controlX = controlX,
                ...
                db.addPuntos(puntosParaGuardar)
